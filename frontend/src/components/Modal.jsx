...
export default function Modal({
  ...
  onClose,
  machineId,
  criterion,         // Das bereits fest vorgegebene Kriterium (z.B. "AVOR")
  allCriteria = [],  // Kann im Hintergrund bleiben
  reasons = {},        // Deine 'reasonOptions' Struktur aus App.jsx
  currentData,
  onSave
}) {
  ...
  // WICHTIG: Nutzt jetzt direkt das fest übergebene 'criterion' aus der Matrix!
  const availableReasons = reasons[criterion] || [];
  ...
  const handleProcessSubmit = (e, targetStatus) => {
    ...
    // WICHTIG: Wir übergeben hier das feste 'criterion' und den korrekten 'targetStatus'
    onSave(machineId, criterion, targetStatus, combinedNote, author.trim() || 'Mitarbeiter');
  };
      ...
      <div style={{
        ...
        <div style={{ padding: '16px' }}>
          ...
          <form onSubmit={(e) => handleProcessSubmit(e, 'red')} style={{ display: 'flex', flexDirection: 'column', gap: '12px' }}>
            ...
            <div>
              <label style={{ display: 'block', fontSize: '11px', fontWeight: '700', color: '#475569', textTransform: 'uppercase', marginBottom: '4px' }}>
                {isCurrentRed ? 'Neues Update' : 'Ergänzende Notiz (Optional)'}
              </label>
              <textarea
                value={note}
            ...
            <div style={{ display: 'flex', justifyContent: 'flex-end', gap: '8px', marginTop: '8px', borderTop: '1px solid #edf2f7', paddingTop: '12px' }}>
              ...
              {isCurrentRed ? (
                <>
                  {/* Bleibt Rot, fügt nur eine Notiz hinzu */}
                  <button type="button" onClick={(e) => handleProcessSubmit(e, 'red')} style={{ padding: '8px 14px', backgroundColor: '#475569', color: '#fff', border: 'none', borderRadius: '6px', cursor: 'pointer', fontSize: '13px', fontWeight: '600' }}>
                    Update hinzufügen
                  </button>
                  {/* Wechselt zurück auf Grün */}
                ...
                /* Speichert eine neue Störung ab (➔ Wechselt zu ROT) */
                <button type="button" onClick={(e) => handleProcessSubmit(e, 'red')} style={{ padding: '8px 14px', backgroundColor: '#ef4444', color: '#fff', border: 'none', borderRadius: '6px', cursor: 'pointer', fontSize: '13px', fontWeight: '600' }}>
                  Als ROT speichern
                </button>
              )}
            </div>
